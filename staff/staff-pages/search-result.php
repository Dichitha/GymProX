...
$qry = "SELECT m.*, 
               (SELECT GROUP_CONCAT(t.task_status) FROM todo t WHERE m.user_id = t.user_id) AS task_status,
               (SELECT GROUP_CONCAT(t.task_desc) FROM todo t WHERE m.user_id = t.user_id) AS task_desc
        FROM members m
        WHERE m.fullname LIKE '%$search%' OR m.username LIKE '%$search%'";
...
if (!$result) {
    ...
    while ($row = mysqli_fetch_array($result)) {
        echo "<tbody> 
                ...
                <td><div class='text-center'>" . formatTodos($row['task_desc'], $row['task_status']) . "</div></td>
            </tbody>";
        $cnt++;
    }
}

function formatTodos($taskDesc, $taskStatus) {
    $todos = explode(',', $taskDesc);
    $statuses = explode(',', $taskStatus);
    $formattedTodos = '';

    foreach ($todos as $index => $todo) {
        $status = $statuses[$index];
        $formattedTodos .= "$todo ($status)<br>";
    }
